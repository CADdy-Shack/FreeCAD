...
static fs::path defaultUserLibrariesRoot()
{
    ...
    return fs::path(home) / "Library" / "Application Support" / "FreeCAD" / "NeuMaterial"
        / "libraries";
#else
...
void MaterialStore::initialize(const fs::path& resourcesPath, const fs::path& userLibrariesRoot)
{
    libraries_.clear();
    byUuid_.clear();

    userLibrariesRoot_ = userLibrariesRoot.empty() ? defaultUserLibrariesRoot() : userLibrariesRoot;
...
std::vector<std::shared_ptr<Material>> MaterialStore::materialsInLibrary(
...
std::shared_ptr<Material> MaterialStore::findByUuid(const std::string& uuid) const
{
...
std::shared_ptr<Material> MaterialStore::addMaterial(Material material, const std::string& libraryName)
{
    if (material.getUuid().empty()) {
        throw std::invalid_argument("Material must have a UUID before calling addMaterial().");
    }
    if (material.getName().empty()) {
        throw std::invalid_argument("Material must have a Name before calling addMaterial().");
    }
...
void MaterialStore::updateMaterial(const Material& material)
{
    if (material.isReadOnly()) {
        throw std::runtime_error("Cannot update read-only material: " + material.getName());
    }

    const auto it = byUuid_.find(material.getUuid());
    if (it == byUuid_.end()) {
        throw std::runtime_error("Material not found in store: " + material.getUuid());
    }
...
void MaterialStore::deleteMaterial(const std::string& uuid)
{
    ...
    if (it->second->isReadOnly()) {
        throw std::runtime_error("Cannot delete read-only material: " + it->second->getName());
    }
...
std::shared_ptr<Material> MaterialStore::duplicateMaterial(
...
)
{
    ...
    if (!source) {
        throw std::invalid_argument("Source material not found: " + sourceUuid);
    }
...
void MaterialStore::discoverLibrary(const Library& lib)
{
    ...
    for (const auto& entry : fs::recursive_directory_iterator(lib.rootPath)) {
        ...
        try {
            ...
            const fs::path rel = fs::relative(entry.path().parent_path(), lib.rootPath);
            if (!rel.empty() && rel != ".") {
...
fs::path MaterialStore::materialFilePath(const Material& material) const
{
    ...
    const fs::path dir = material.getCategory().empty() ? lib->rootPath
